...
const EventInfo: React.FC<EventInfoProps> = ({ game, status }) => {
  ...
  return (
    <div className="bg-bg-l1 rounded-lg shadow-lg p-3">
      <div className="flex items-center justify-center text-grey-60 text-caption-12 font-medium py-2 border-b border-b-grey-10">
        <Icon className="size-4" name={`sport/${sportSlug}` as IconName} />
        <Flag className="mr-1 border border-grey-10 -ml-1" country={countrySlug} />
        <span>{countryName}</span>
        <div className="size-[2px] mx-1 bg-grey-70 rounded-full" />
        <span>{leagueName}</span>
      </div>
      <div className="ds:py-4 mb:py-2 w-full mb:px-4">
        <div className="relative flex items-center ds:justify-around mb:justify-center ds:max-w-[50%] mx-auto">
          <OpponentLogo className="mb:absolute mb:left-0 mb:top-0" image={getParticipantImage(participants[0].name, participants[0].image || '')} size={48} />
          <Title status={status} startsAt={startsAt} />
          <OpponentLogo className="mb:absolute mb:right-0 mb:top-0" image={getParticipantImage(participants[1].name, participants[1].image || '')} size={48} />
        </div>
        <div className="text-center ds:text-heading-h3 mb:text-heading-h5 font-bold mt-2">{title}</div>
      </div>
      ...
      {/* Removed the HeadToHead component here */}
    </div>
  )
}
...
export const EventInfoSkeleton: React.FC = () => (
  <div className="animate-pulse -mx-2">
    ...
    <div className="ds:py-4 mb:py-2 w-full mb:px-4">
      ...
      <div className="h-6 bg-bg-l2 rounded w-3/4 mx-auto mt-2" />
    </div>
  </div>
)
