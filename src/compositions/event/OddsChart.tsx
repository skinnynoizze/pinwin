...
// Update the ChartDataPoint type to allow null values
type ChartDataPoint = {
  date: string;
...
const OddsChart: React.FC<OddsChartProps> = ({ gameId, isLoading, setIsLoading }) => {
  ...
  const generateDynamicTicks = (data: ChartDataPoint[], tickCount: number): string[] => {
    ...
    if (allDates.length <= tickCount) return allDates;
    ...
    ticks.push(allDates[allDates.length - 1]); // Always include the last date (today)
  ...
  const dynamicTicks = generateDynamicTicks(extendedChartData, 6); // Aim for 6 ticks including today
      ...
      <ResponsiveContainer width="100%" height={400}>
        ...
        >
          ...
          <Tooltip 
            ...
            formatter={(value) => value ? Number(value).toFixed(3) : 'N/A'}
          />
          <Legend 
            layout="horizontal"
            align="center"
