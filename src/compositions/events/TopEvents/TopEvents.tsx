...
const TopEvents: React.FC<TopEventsProps> = ({ sportSlug }) => {
  ...
  const { games, loading } = useGames({
    filter: { limit: 200, ...(sportSlug ? { sportSlug } : {}) }, // Only apply sportSlug filter if it exists
    orderBy: Game_OrderBy.Turnover,
  })
      ...
      <div className="px-4 flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className="w-8 h-6 flex items-center justify-center bg-gray-800 rounded-tl-full rounded-tr-1 rounded-br-1 rounded-bl-full border border-grey-15 text-grey-60 hover:text-brand-50 transition"
          >
            <Icon className="size-5" name="interface/chevron_left" />
          </button>
          <button
            onClick={() => moveCarousel('next')}
            className="w-8 h-6 flex items-center justify-center bg-gray-800 rounded-tl-1 rounded-tr-full rounded-br-full rounded-bl-1 border border-grey-15 text-grey-60 hover:text-brand-50 transition"
          >
